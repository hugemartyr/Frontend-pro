...
function Orders() {
    ...
    const [displySkins, setDisplaySkins] = useState<OrderItem1[] | null>(null);
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const skinIds: number[] = await skinIdContract.methods.getGameSkins().call();
                console.log("skinIds: ", skinIds);

                if (currentUser) {
                    console.log(currentUser.name);
                    const userSkinsPromises = skinIds.map(async () => {
                        return await skinOwnerCon.methods.getUserSkins(currentUser.name).call();
                    });

                    const t = (await Promise.all(userSkinsPromises)).flat();
                    console.log(t);
                    ...
                    const filteredAndMappedSkins = skinCards.skins
                        ...
                        .map((x) => ({
                            ...x,
                            player_name: currentUser.name || 'Unknown'
                        }));
						console.log("filteredAndMappedSkins: ", filteredAndMappedSkins);
