	...
	(async () => {
		try {
			...
			const t:number[] = await skinOwnerCon.methods
					...
					if(t[8] === Number(skinCards.skins[1].idx)){
						console.log("include:", t.includes(Number(skinCards.skins[1].idx)));
						console.log("t.type: ", typeof(t[8]), "");
					}

					const filteredAndMappedSkins = skinCards.skins
						.filter((x) => t.includes(Number(x.idx)))
						// .map((x) => ({
						//     ...x,
						//     player_name:'Ajitesh'
						// }));
