...
import { create } from 'zustand';
import skinOwnershipABI from "../components/abis/skinOwnershipABI.json";
import { getAccounts ,skinMarket,skinOwner} from '../utils/web3';
...
// Function to fetch user skins and transform the data accordingly
async function getSkins() {   
    const userName = "Ajitesh";
    const skinOwnership = skinOwner();
    const userSkins = await fetchUserSkins(userName, skinOwnership);
    console.log("userSkins: ", userSkins);

    const userOrderItems = example.filter((x) => userSkins.includes(x.idx));
    userOrderItems.forEach(item => item.player_name = userName);

    return { orderItems: userOrderItems };
}
...
const useOrderItems = create<OrderItemState>((set) => {
    ...
    // Fetch the skins and update the state
    getSkins().then((skinsState) => {
        set(skinsState);
    });
